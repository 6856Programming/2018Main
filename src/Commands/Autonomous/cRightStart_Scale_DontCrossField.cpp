#include "cRightStart_Scale_DontCrossField.h"
...
cRightStart_Scale_DontCrossField::cRightStart_Scale_DontCrossField()
{
	...
	// 3. Hold lift
	// 4. We're RIGHT, to turn 45 degrees (COUNTER CLOCKWISE)
...
//	::SmartDashboard::PutString("Scale Location", ::g_GameState.DecodePositionEnumString(nearScaleLocation) );
	...
	if ( nearScaleLocation != cGameState::RIGHT )
	{
		...
		::SmartDashboard::PutString("Scale", "Scale is on the LEFT, so not dropping");

		return;
	}
	// **********************************************

	::SmartDashboard::PutString("Scale", "Scale is on the RIGHT, so DROPPING");
	...
	{
		sMovementParamHelper driveState;
		driveState.totalDistance = 45.0;		// +ve, so COUNTER CLOCKWISE
